...
output_dir = Path(f"/media/user/Expansion/PRISM/output/nc/{measure}/")
output_dir.mkdir(parents=True, exist_ok=True)

files = sorted(list(data_dir.rglob("*.parquet")))

nc = dd.read_parquet("/media/user/Expansion/PRISM/output/nc/nc_prism_id.parquet")[
    ["id"]
].persist()
...
for f in files:
    ...
    ddf = dd.read_parquet(f).merge(nc, on="id", how="inner").repartition(npartitions=1)
