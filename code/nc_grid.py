...
nc = (
    gpd
    .read_file("input/tl_2025_us_state/tl_2025_us_state.shp")
...
id_gdf = pd.read_parquet("output/us/us_prism_id.parquet").pipe(
    lambda x: gpd.GeoDataFrame(
        x, 
        geometry=gpd.points_from_xy(x.x, x.y), 
        crs=rast_crs
    )
).to_crs(nc.crs)
...
nc_id = (
    id_gdf
    .sjoin(nc, how="inner", predicate="within")
